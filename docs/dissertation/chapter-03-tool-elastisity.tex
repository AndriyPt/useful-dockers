...
розв'язок рівняння пружності (\ref{eqn:elasticity_2d_eqn}) для $\xi \in \Omega_1$, де $\Gamma = \partial\Omega_1$:

\begin{equation}
    \label{eqn:elasticity_2d_bem_direct_elasticity}
    c_{ij}(\xi)u_j(\xi) + \int_{\Gamma}u_j(x)\hat{q_{ij}}(x,\xi)d\Gamma(x) = 
        \int_{\Gamma}q_j(x)\hat{u_{ij}}(x,\xi)d\Gamma(x) + \int_{\Omega_3} + !!!
\end{equation}

\noindent$\hat{u}$ і $\hat{q}$ фундаментальний розв'язок плоского рівняння пружності і його похідна по нормалі 
відповідно.
