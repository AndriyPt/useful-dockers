...
Розглянемо область у якій є дві частини (рис. \ref{fig:elasticity_3d_domain}). $\Omega_1$ відповідає м'яким тканинам, а
$\Omega_3$ - це злоякісне утворення. Область $\Omega_2$ - це хірургічний інструмент, який контактує із м'якими тканинами
по межі $\Gamma_2$, тобто $\Gamma_2=\partial\Omega_2 \cap \partial\Omega_1$, 
$\Gamma_1 = \partial \Omega_1 \backslash \Gamma_2$.
...
\begin{figure}[ht!]
    ...
    \caption{Переріз тканин і інструмента}
    \label{fig:elasticity_3d_domain}
\end{figure}
...
\noindent На всій межі $\Gamma_1$ немає переміщень
\begin{equation}
    \label{eqn:elasticity_3d_cond_1}
    u_i(x) = 0, x \in \Gamma_1
\end{equation}

\noindent На всій межі $\Gamma_2$ діє хірургічний інструмент вздовж нормалі із компонентами $n_j$ і зусиллям
\begin{equation}
    \label{eqn:elasticity_3d_cond_2}
    t_i(x) = t_{pi}, x \in \Gamma_2
\end{equation}
...
\begin{equation}
    \label{eqn:elasticity_3d_lame_lambda_convertion}
    \lambda = \frac{E\nu}{(1+\nu)(1-2\nu)}
\end{equation}

\begin{equation}
    \label{eqn:elasticity_3d_lame_mu_convertion}
    \mu = \frac{E\nu}{2(1+\nu)}
\end{equation}
...
Обчислимо коефіцієнти Ляме за формулами \ref{eqn:elasticity_3d_lame_lambda_convertion} і 
\ref{eqn:elasticity_3d_lame_mu_convertion}. Враховуючи неоднорідності отримаємо наступне представлення:

\begin{equation}
    \label{eqn:elasticity_3d_lyame_lambda}
    \lambda(x) = \lambda_c(x)\chi_3 + \lambda_t, x \in \Omega_1
\end{equation}
...
\begin{equation}
    \label{eqn:elasticity_3d_lyame_mu}
    \mu(x) = \mu_c(x)\chi_3 + \mu_t, x \in \Omega_1
\end{equation}
...
Рівняння \ref{eqn:elasticity_3d_eqn} перепишемо у наступному вигляді:

\begin{equation}
    \label{eqn:elasticity_3d_eqn_with_f}
    \mu_t u_{i,jj}(x) + (\lambda_t + \mu_t)u_{j,ji}(x) = -f_{ij,j}(\varepsilon(x)), x \in \Omega_1
\end{equation}
...
Отже, для визначення переміщень маємо змішану крайову задачу \ref{eqn:elasticity_3d_eqn_with_f},
\ref{eqn:elasticity_3d_cond_1} і \ref{eqn:elasticity_3d_cond_2} із коефіцієнтами Ляме у вигляді
\ref{eqn:elasticity_3d_lyame_lambda} і \ref{eqn:elasticity_3d_lyame_mu}.

\subsection{Дискретизація рівнянь}

\textbf{Інтегральні зображення розв'язків.} На основі прямого методу граничних елементів, як показано у
\cite{brebbia-bem}, опишемо розв'язок рівняння пружності (\ref{eqn:elasticity_3d_eqn_with_f}) для $\xi \in \Omega_1$, де
$\Gamma = \partial\Omega_1$:

\begin{equation}
    \label{eqn:elasticity_3d_bem_direct_elasticity}
    \begin{split}
    u_i(\xi) = \int_{\Gamma}\hat{u}_{ij}(x,\xi)p_j(x)d\Gamma(x) - \int_{\Gamma}\hat{p}_{ij}(x,\xi)u_j(x)d\Gamma(x) + \\  
        + \int_{\Omega_3} \hat{u}_{ij}(x,\xi)f_{jk,k}(\varepsilon(x))d\Omega(x)
    \end{split}
...
\noindent$\hat{u}_{ij}$ і $\hat{p}_{ij}$ фундаментальні розв'язки плоского рівняння пружності для переміщень і напружень 
відповідно. Застосуємо до \ref{eqn:elasticity_3d_bem_direct_elasticity} теорему Гріна для інтегрування частинами. 
Одержимо:

\begin{equation}
    \label{eqn:elasticity_3d_bem_direct_elasticity_simplified}
    \begin{split}
    u_i(\xi) = \int_{\Gamma}\hat{u}_{ij}(x,\xi)p_j(x)d\Gamma(x) - \int_{\Gamma}\hat{p}_{ij}(x,\xi)u_j(x)d\Gamma(x) + \\ 
        + \int_{\Gamma_3} \hat{u}_{ij}(x,\xi)f_{jk}(\varepsilon(x))n_kd\Gamma(x)
        - \int_{\Omega_3} \hat{u}_{ij,k}(x,\xi)f_{jk}(\varepsilon(x))d\Omega(x)
...
\begin{equation}
    \label{eqn:elasticity_3d_bem_direct_gauchy_tensor}
    \begin{split}
    \varepsilon_{ij}(\xi) = \int_{\Gamma}\tilde{u}_{ijk}(x,\xi)p_k(x)d\Gamma(x) 
        - \int_{\Gamma}\tilde{p}_{ijk}(x,\xi)u_k(x)d\Gamma(x) + \\ 
        + \int_{\Gamma_3} \tilde{u}_{ijk}(x,\xi)f_{kl}(\varepsilon(x))n_l d\Gamma(x)
...
$\left.\varepsilon(\eta_1, \eta_2)\right|_{\Omega_3^l} = \psi^T(\eta_1,\eta_2)\varepsilon^l$. $\phi(\eta_1)$ і 
$\psi(\eta_1, \eta_2)$ --- інтерполяційні функції в локальних координатах для границі $\Gamma$ і області $\Omega_3$. 
Рівняння \ref{eqn:elasticity_3d_bem_direct_elasticity_simplified} і \ref{eqn:elasticity_3d_bem_direct_gauchy_tensor} 
представимо на основі інтерполяційних функцій наступним чином:
...
\begin{figure}[ht!]
    ...
    \end{tikzpicture}
    \caption{Тиск на поверхні м'якої тканини залежно від розмірів включення}
    \label{fig:elasticity_3d_solution_border_pressure_size}
\end{figure}
