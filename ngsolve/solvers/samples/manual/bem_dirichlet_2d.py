...
class MeshLoader:
    ...
    @staticmethod
    def generate_mesh_from_file(filename: str, max_element_size: float, scale_x: float = 1.0, scale_y: float = 1.0):
        ...
        with tempfile.NamedTemporaryFile(suffix=".msh") as tmp:
            ...
            result.points = result.points @ A.T
    ...
    def is_corner_point(self, point: np.array):
        ...
        for item in self.get_border():
            if item.is_right_corner and Utils.is_the_same_point(
                item.element[1], point, Domain2D.POINT_LOCATION_EPSILON
            ):
                result = True
                break
        return result
...
class GmshDomain2D(Domain2D):
    ...
    def __init_corner_points(self):
        ...
        for index, element in enumerate(self.__border):
            ...
            if not Utils.is_the_same_vector(element.normal, next_element.normal, GmshDomain2D.POINT_LOCATION_EPSILON):
    ...
    class Laplace:
        ...
        class BEM:
            ...
            def init_dirichlet_square():
                print("BEM for Dirichlet problem for Laplace equation in square...")
            ...
            def init_dirichlet_single_inclusion_square():
                print("BEM for Dirichlet problem for Laplace equation with single square inclusion...")
                ...
                inclusion_domain = GmshDomain2D(
                    MeshLoader.generate_mesh_from_file(
                        MeshLoader.UNIT_SQUARE_FILE,
                        GlobalSettings.INCLUSION_ELEMENTS_MAX_SIZE,
                        scale_x=INCLUSION_SIZE,
                        scale_y=INCLUSION_SIZE,
                    ),
                    MeshLoader.order_conditions(
                        {
        ...
        class CoBEM:
            ...
            def init_dirichlet_hexagon():
                ...
                domain = GmshDomain2D(
                    MeshLoader.generate_mesh_from_file(
                        MeshLoader.UNIT_HEXAGON_FILE, GlobalSettings.BORDER_ELEMENT_MAX_SIZE, scale_x=2.0, scale_y=2.0
                    ),
                    MeshLoader.order_conditions(
                        {
                            MeshLoader.TOP: (BoundaryConditionType.DIRICHLET, dirichlet_boundary_value),
            ...
            @staticmethod
            def init_dirichlet_square():
                print("CoBEM for Dirichlet problem for Laplace equation in square...")
    ...
    class Pennes:
        ...
        class CoBEM:
            ...
            def init_dirichlet_hexagon():
                ...
                domain = GmshDomain2D(
                    MeshLoader.generate_mesh_from_file(
                        MeshLoader.UNIT_HEXAGON_FILE, GlobalSettings.BORDER_ELEMENT_MAX_SIZE, scale_x=3.0, scale_y=2.0
                    ),
                    MeshLoader.order_conditions(
                        {
                            MeshLoader.TOP: (BoundaryConditionType.DIRICHLET, dirichlet_boundary_value),
